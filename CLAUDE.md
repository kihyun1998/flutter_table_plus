...
- **"upstream 으로 간다" 는 "upstream 탓을 한다" 가 아니다.** 물을 것은 **누구의 불변식이 깨졌는가**다 — 그리고 **그 답을 이쪽이 최종적으로 내리는 건 아니다.** 실증(#88 → #96): `MouseRegion.onEnter` 에서 조상 툴팁을 억제하는 건 0.4.0(#22)이 *의도한 계약*이라 판정하고, 그릴 게 없는 툴팁을 지은 이쪽을 고쳤다. 가드는 잘 들었다. **그래서 upstream 에 보고하지 않았다.** just_tooltip 0.4.4 는 같은 지점을 자기 결함으로 판정하며 적는다 — *"Two packages had independently grown the same local guard rather than report it, which is what a trap in the upstream default looks like."* 바로 아래 #33 항목에서 배운 문장의 재연이다. **판정과 보고는 별개의 의무다.** 고칠 자리를 여기로 판단했더라도 보고는 한다. 보고하지 않으면 upstream 은 판정할 기회조차 없고, 잘 드는 우회는 그 침묵을 영원히 정당해 보이게 만든다.
...
- **Tooltip Control**: Fine-grained tooltip behavior control for both cells and headers via `tooltipBehavior` and `headerTooltipBehavior` properties. Two kinds of body tooltip, gated differently: a **text** tooltip exists to reveal truncated glyphs, so it is gated on ellipsis / non-empty text and its hover target is the `Text` itself. A **widget** tooltip (`tooltipBuilder`) draws unrelated content, so only `TooltipBehavior.never` suppresses it and its hover target is the whole cell. `TooltipResolver.shouldShow` takes `hasWidgetTooltip` to keep the two apart. A **row** tooltip (`rowTooltipBuilder`) wraps the whole row in `table_body.dart`'s `itemBuilder` with `TooltipAnchor.pointer` — hover region and anchor must differ, since a row is `contentWidth` wide. Nesting is arbitrated by just_tooltip (innermost wins), so this package holds no priority logic. Since just_tooltip 0.4.4 "innermost" means **the innermost tooltip that has something to draw**: an empty-message tooltip under `hideOnEmptyMessage` is inert — built, silent, and it yields the row card. That is why `^0.4.4` is a **floor, not a preference**. 0.4.3 claimed the ancestor at `MouseRegion.onEnter` and only afterwards declined to draw, so hovering such a cell showed nothing at all; this package answered by never building a tooltip that could not show (#88), and that guard is now gone (#96) with the floor standing in its place. `table_header_cell.dart`'s `label.isEmpty` is a different thing and stays — it is a `shouldShow` policy, and a header tooltip nests inside nothing. Beware: `TooltipBehavior.always` gives every ellipsized column a tooltip whether or not its text is cut, which leaves a row card nowhere to appear. A row's hover region does cover empty cells — `RenderMouseRegion` is opaque and `hitTestSelf`s, so a zero-width `Text` under the pointer is irrelevant; do not "fix" that by making rows hit-test opaque
