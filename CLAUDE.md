...
### Testing
```bash
flutter test                    # Package suite (378 tests)
flutter test test/row_tooltip_test.dart         # One file
cd example && flutter test      # Example suite (28 tests) — a gate, not an afterthought
```
...
flutter test                                       # 378
cd example && flutter analyze && flutter test      # 28
...
- **태그는 문서·example 까지 다 들어간 뒤에 단다.** 발행되지 않은 태그를 옮기는 비용은 0 이다. 실증(2.15.0): 머지되지 않은 PR #69 의 존재를 모른 채 `main` 끝에 태그했고, 그 트리는 `^0.4.0` + `flutter >=3.10.0` 조합이라 3.10~3.12 사용자에게 깨지는 상태였다. pub.dev 에 안 올렸고 GitHub Release 도 없어서 태그를 지우고 다시 달 수 있었다. **릴리스 전에 열린 PR 과 로컬 브랜치를 확인한다.**
