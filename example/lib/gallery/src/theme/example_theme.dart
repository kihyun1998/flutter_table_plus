...
/// to use. Borrowing it for the chrome adds no dependency and no asset.
///
/// **It is a Latin subset**, generated by `scripts/fonts/subset_pretendard.py`
/// — which is where the charset is written down, as Unicode ranges. Read it
/// there rather than trusting a list here; a second copy of a charset is
/// somewhere for the two to disagree, and the last time this file described the
/// subset from memory it described a different font.
///
/// This comment used to say the subset was bundled because *"it carries Korean
/// text — which the demo data contains"*. Both halves were false: the subset has
/// never had a Hangul glyph, and a `[가-힣]` scan of `example/lib` returns
/// nothing. The claim survived six commits because a missing glyph does not
/// throw — Flutter draws it from a platform face, so wrong text renders as a
