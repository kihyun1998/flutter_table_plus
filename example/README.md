...
It is syntax-highlighted and there is a copy button, and there are two ways to get the file out —
proved separately, because they are different paths. The **button** copies the bundle's bytes
directly and never passes through the highlighter at all. **Selecting and copying** does pass
through it, so the tokenizer returns a *partition*: concatenating what is drawn reproduces the file
exactly, and the test flattens the rendered spans with the same call the clipboard makes. There are
no line numbers — this is not a source viewer, it is the paste.
