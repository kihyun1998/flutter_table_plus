...
*   **FEAT**: `TableRowHeightCalculator` can now measure what the glyphs actually get. Three inputs were invisible to it, all in the same direction, and a row sized from them is one the text does not fit inside
    ...
    *   **A performance defect the docs were teaching, now documented rather than repaired.** `createHeightCalculator` is shown inline in the widget's constructor, and the height caches — plus the row geometry every drag hit-test is answered from — drop whenever the callback is not `==` to the previous one. Measured: two calls with identical arguments return callbacks that compare `!=`, so the inline form re-measures every row on every frame. **Hold it in a field.** A value type with an `==` does not fix this, which was tried and measured: Dart compares two tear-offs of the same method by whether their receivers are `identical`, never by whether they are `==`. The remaining fix changes the type of `FlutterTablePlus.calculateRowHeight`, which is breaking, so it is not in this release
