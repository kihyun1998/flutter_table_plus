...
*   **INTERNAL**: `lib/` changed in exactly four places this release — the three fixes above and nothing else. (This line read "one place", then "three places", as each further fix landed under the same open version. It is left growing rather than made vague: a number that has to move is what shows the version is still open.)
