...
*   **INTERNAL**: `lib/` changed in the three fixes above, in the removal at the top of this entry, and in doc-comments: one correction that #128's sweep reclaimed, plus the three field contracts #132 wrote — the cached-geometry field still described the invalidation set #120 had already widened. (This line read "one place", then "three places", then "four places", as each further change landed under the same open version. It has stopped carrying a count: the count was reassurance that a *patch* release was small, and this is no longer a patch release. The list is the honest form.)
