...
*   **INTERNAL**: `lib/` changed in the three fixes above, and in the removal at the top of this entry. (This line read "one place", then "three places", then "four places", as each further change landed under the same open version. It has stopped carrying a count: the count was reassurance that a *patch* release was small, and this is no longer a patch release. The list is the honest form.)
